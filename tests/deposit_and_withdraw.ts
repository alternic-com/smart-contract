...
describe("escrow", () => {
  ...
  it("Withdraw", async () => {
    try {
      ...
      const { maker, taker, mintDomain, mintCurrency, makerAtaDomain, makerAtaCurrency, takerAtaA, takerAtaCurrency } = global.escrowSetup;
      const { escrowPda, vaultPda, seed, depositAmount, receiveAmount } = global.escrowTest;

      console.log("=== Cancel INSTRUCTION ===");
      console.log("Escrow PDA:", escrowPda.toBase58());
      console.log("Vault PDA:", vaultPda.toBase58());
      console.log("Expected to transfer:", depositAmount.toString(), "NFT Domain to maker");
      console.log("Expected to receive:", depositAmount.toString(), "NFT Domain from vault");

      // Get initial balances
      const [makerAtaDomainBefore, makerAtaCurrencyBefore, vaultBefore, takerAtaABefore, takerAtaCurrencyBefore] = await Promise.all([
        getAccount(provider.connection, makerAtaDomain),
        getAccount(provider.connection, makerAtaCurrency),
        getAccount(provider.connection, vaultPda),
        getAccount(provider.connection, takerAtaA),
        getAccount(provider.connection, takerAtaCurrency),
      ]);

      console.log("\n=== INITIAL BALANCES ===");
      console.log("Maker's NFT Domain balance:", Number(makerAtaDomainBefore.amount));
      console.log("Maker's Token Currency balance:", Number(makerAtaCurrencyBefore.amount));
      console.log("Vault NFT Domain balance:", Number(vaultBefore.amount));
      console.log("Taker's NFT Domain balance:", Number(takerAtaABefore.amount));
      console.log("Taker's Token Currency balance:", Number(takerAtaCurrencyBefore.amount));
  ...
  .accounts({
    ...
    associatedTokenProgram: anchor.utils.token.ASSOCIATED_PROGRAM_ID,
      ...
      console.log(`\nCancel transaction signature: https://explorer.solana.com/transaction/${signature}?cluster=custom&customUrl=${provider.connection.rpcEndpoint}`);

      // Get final balances
      const [makerAtaDomainAfter, makerAtaCurrencyAfter, vaultAfter, takerAtaAAfter, takerAtaCurrencyAfter] = await Promise.all([
        getAccount(provider.connection, makerAtaDomain),
        getAccount(provider.connection, makerAtaCurrency),
        getAccount(provider.connection, vaultPda),
        getAccount(provider.connection, takerAtaA),
      ...
      console.log("\n=== FINAL BALANCES ===");
      console.log("Maker's NFT Domain balance:", Number(makerAtaDomainAfter.amount));
      console.log("Maker's Token Currency balance:", Number(makerAtaCurrencyAfter.amount));
      console.log("Vault NFT Domain balance:", Number(vaultAfter.amount));
      console.log("Taker's NFT Domain balance:", Number(takerAtaAAfter.amount));
      console.log("Taker's Token Currency balance:", Number(takerAtaCurrencyAfter.amount));
      ...
      console.log("\n=== MAKER BALANCE VERIFICATION ===");
      console.log("Maker's NFT Domain balance after:", Number(makerAtaDomainAfter.amount));
      assert.equal(Number(makerAtaDomainAfter.amount), Number(makerAtaDomainBefore.amount) + Number(depositAmount), "Maker didn't received all its A");

      console.log("Maker's Token Currency balance after:", Number(makerAtaCurrencyAfter.amount));
      assert.equal(Number(makerAtaCurrencyAfter.amount), 0, "Maker received some B despite cancellation :/");

      console.log("\n=== TAKER BALANCE VERIFICATION ===");
      console.log("Taker's NFT Domain balance after:", Number(takerAtaAAfter.amount));
      console.log("Taker's Token Currency balance after:", Number(takerAtaCurrencyAfter.amount));
      assert.equal(Number(takerAtaAAfter.amount), Number(takerAtaABefore.amount), "Taker did receive some A");
      assert.equal(Number(takerAtaCurrencyAfter.amount), Number(takerAtaCurrencyBefore.amount), "Taker did receive some B");

      console.log("\n✅ Cancel instruction executed successfully!");
